...
data = pd.read_csv("4_diary_main.csv", encoding="latin-1", usecols=[
                   "ID", "BEGIN", "Q401"], converters={"Q401": transfer_code, "BEGIN": transfer_time})
data = data.sort_values(["ID", "BEGIN"])
data = data.rename(
    columns={"ID": "sample_id", "BEGIN": "time", "Q401": "activity"})
data = data.iloc[:int(len(data)/5)]
d3 = D3Blocks()
d3.movingbubbles(data, datetime="time", state="activity", sample_id="sample_id",
                 filepath="output.html", center="Travelling", note="Vietnamese people's daily rountine", cmap="hsv", figsize=(780, 800), size=2)
